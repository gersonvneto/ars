...
#rt = read_twitter_csv("tweets_coleta_2018_11_08_16_17_25.csv")
...
# tirando o celso portiolli
#texto = tm_map(texto, removeWords, c("celso","portiolli"))


# tentativa de stem (cortar as palavras com o mesmo radical)
library(SnowballC)
...
# para cada documento, ele vai marcando quantas vezes as palavras aparecem (uma matriz 2-mode)
tdm = TermDocumentMatrix(texto)
tdm = removeSparseTerms(tdm, .94) # retira termos esparsos
dim(tdm) # confere as dimensões

# Cria a rede
g <- graph_from_incidence_matrix(as.matrix(tdm)) # criando uma matriz de afiliacao (retangular)
...
plot(p, vertex.label.cex=deg/10, edge.width=(E(p)$weight)/120, 
